...
/**
 * Sends mail through the Snailmark HTTP API (POST /api/email, bearer-token
 * auth). The base URL is configurable so it can point at snailmark.com or a
 * self-hosted Snailmark instance.
 */
class SnailmarkTransport implements TransportInterface
{
    ...
    public function __construct(
        ...
        protected string $baseUrl = 'https://api.snailmark.com',
    ) {
    }
    ...
    public function send(RawMessage $message, ?Envelope $envelope = null): ?SentMessage
    {
        ...
        $response = $this->http
            ...
            ->acceptJson()
            ->withToken($this->getServerToken($email))
        ...
        throw new SnailmarkTransportException(
            $response->json('Message') ?? $response->reason(),
            (int) ($response->json('ErrorCode') ?? 0),
    ...
    {
        $url = rtrim($this->baseUrl, '/').'/api/email';
    ...
    protected function getPayload(Email $email, Envelope $envelope): array
    {
        ...
        foreach ($email->getHeaders()->all() as $name => $header) {
            ...
            if ($header instanceof SnailmarkServerTokenHeader) {
                continue;
            }
    ...
    /**
     * The per-message token header wins over the configured token, so a single
     * app can send on behalf of multiple Snailmark servers.
     */
    protected function getServerToken(Email $email): ?string
    {
        $header = $email->getHeaders()->get(SnailmarkServerTokenHeader::NAME);
    ...
    /**
     * Snailmark templated sends are triggered by encoding a JSON payload
     * ({"alias": "...", "model": {...}}) as the HTML body.
     */
